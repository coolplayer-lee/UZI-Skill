...
def fetch_xueqiu_cubes(ti, limit: int = 50) -> list[dict]:
    sess = _xq_session()
    ...
    try:
        ...
        return [{"error": f"xueqiu fetch failed: {e}"}]
...
def main(ticker: str) -> dict:
    ...
    xq = cached(ti.full, f"xq_cubes__{ti.code}", lambda: fetch_xueqiu_cubes(ti), ttl=6 * 3600)
    time.sleep(0.5)
    ...
    return {
        ...
        "data": {
            ...
            "_note": "雪球 cubes API 是主数据源；其余 3 站点 ≥1 失败时 Claude 用 fallback_queries 补足",
        },
